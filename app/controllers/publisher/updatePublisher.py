        ...
        if currentUser['role'] == "Admin" or currentUser['role'] == "User":
            ...
            for key in requestStruct.Publisher():
                if key not in jsonBody:
                    ...
                    return responseHandler.badRequest(response)
            checkName = db.select(f"select name from tbl_book_publisher where name = '{result['name']}' and name != (select name from tbl_book_publisher where id_book_publisher = '{id}')")
            if checkName:
                response = {
                    "Message": "Publisher Name is Exist"
                }
                return responseHandler.badRequest(response)
            checkEmail = db.select(f"select email from tbl_book_publisher where email = '{result['email']}' and email != (select email from tbl_book_publisher where id_book_publisher = '{id}')")
            if checkEmail:
                response = {
                    "Message": "Email is exist"
                }
                return responseHandler.badRequest(response)
            Publisher[id].set(name = result['name'],
                           email = result['email'],
